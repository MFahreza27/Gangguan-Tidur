    ...
    <div class="content-wrapper">
        <div class="row">
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">FORM EDIT DATA GEJALA</h4><br>
                        <form action="{{ route('admin.gejala.update',['id' => $data->id]) }}" method="POST" class="forms-sample">
                            @csrf
                            <div class="form-group row">
                                <label for="exampleInputUsername2" class="col-sm-3 col-form-label">Kode</label>
                                <div class="col-sm-9">
                                    <input type="text" name="kode" value="{{ $data->kd_gejala }}" class="form-control"  placeholder="Kode" readonly>
                                    @error('nama')
                                        <small>{{ $message }}</small>
                                    @enderror
                                </div>
